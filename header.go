...
// ReadFromBuffer read data from  temporary buffer
func ReadFromBuffer(buf []byte)(*Header,[]byte,error){
	if bytes.Equal(buf[:1], SIGV1[:1]) || bytes.Equal(buf[:1], SIGV2[:1]){
		signature1 := buf[:5]
		if bytes.Equal(signature1[:5], SIGV1) {
			return parseVersion1ByBuf(buf)
		}
	}
	return nil, buf,ErrNoProxyProtocol
